...
def mamba_chunk_scan_combined_torch(
    ...
    y, state = ssd_minimal_discrete(
        u, w, B, C, block_len=chunk_size, initial_states=initial_states
    )
    if D is not None:
...
def selective_scan_chunk_fn(
    ...
    fn = (
        mamba_chunk_scan_combined_torch
        if backend == "torch" or (not WITH_TRITON)
