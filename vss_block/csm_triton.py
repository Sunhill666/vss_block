...
def triton_cross_scan_1b1(
    ...
    for idxc in range(_for_C):
        _idx = idxc * DH * DW
        tl.store(p_y1 + _idx, tl.load(p_x1 + _idx,
                 mask=_mask_hw), mask=_mask_hw)
        tl.store(p_y2 + _idx, tl.load(p_x2 + _idx,
                 mask=_mask_hw), mask=_mask_hw)
        tl.store(p_y3 + _idx, tl.load(p_x3 + _idx,
                 mask=_mask_hw), mask=_mask_hw)
        tl.store(p_y4 + _idx, tl.load(p_x4 + _idx,
                 mask=_mask_hw), mask=_mask_hw)
...
class CrossScanTriton1b1(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor):
        ...
        x = x.contiguous()
        y = x.new_empty((B, 4, C, H, W))
        triton_cross_scan_1b1[(NH * NW, NC, B)](x, y,
    ...
    def backward(ctx, y: torch.Tensor):
        ...
        triton_cross_merge_1b1[(NH * NW, NC, B)](x, y,
                                                 BC, BH, BW, C, H, W, NH, NW)
        return x
...
class CrossScanTritonF(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor, channel_first=True):
        ...
        if not channel_first:
            B, H, W, C = x.shape
        B, C, H, W = int(B), int(C), int(H), int(W)
        BC, BH, BW = min(triton.next_power_of_2(C), 1), min(
            triton.next_power_of_2(H), 64), min(triton.next_power_of_2(W), 64)
        NH, NW, NC = triton.cdiv(H, BH), triton.cdiv(W, BW), triton.cdiv(C, BC)
        ...
        x = x.contiguous()
        y = x.new_empty((B, 4, C, H, W))
        triton_cross_scan_flex[(
            NH * NW, NC, B)](x, y, (0 if channel_first else 1), 0, 0, 0, BC, BH, BW, C, H, W, NH, NW)
        return y.view(B, 4, C, -1)
    ...
    def backward(ctx, y: torch.Tensor):
        ...
        BC, BH, BW, NC, NH, NW = ctx.triton_shape
        y = y.contiguous().view(B, 4, C, H, W)
        x = y.new_empty((B, C, H, W)) if channel_first else y.new_empty(
            (B, H, W, C))
        triton_cross_scan_flex[(
            NH * NW, NC, B)](x, y, (0 if channel_first else 1), 0, 1, 0, BC, BH, BW, C, H, W, NH, NW)
        return x, None
...
class CrossScanTriton1b1F(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor, channel_first=True):
        ...
        y = x.new_empty((B, 4, C, H, W))
        triton_cross_scan_flex[(
            NH * NW, NC, B)](x, y, (0 if channel_first else 1), 0, 0, 1, BC, BH, BW, C, H, W, NH, NW)
        return y.view(B, 4, C, -1)
    ...
    def backward(ctx, y: torch.Tensor):
        ...
        BC, BH, BW, NC, NH, NW = ctx.triton_shape
        y = y.contiguous().view(B, 4, C, H, W)
        x = y.new_empty((B, 4, C, H, W)) if channel_first else y.new_empty(
            (B, H, W, 4, C))
        triton_cross_scan_flex[(
            NH * NW, NC, B)](x, y, (0 if channel_first else 1), 0, 1, 1, BC, BH, BW, C, H, W, NH, NW)
        return x, None
...
def getCSM(mode=1):
    ...
    class CrossScanTriton(torch.autograd.Function):
        ...
        def forward(ctx, x: torch.Tensor):
            ...
            B, C, H, W = int(B), int(C), int(H), int(W)
            BC, BH, BW = 1, 32, 32
            NH, NW, NC = triton.cdiv(H, BH), triton.cdiv(
                W, BW), triton.cdiv(C, BC)
            ctx.shape = (B, C, H, W)
            ctx.triton_shape = (BC, BH, BW, NC, NH, NW)
            x = x.contiguous()
            y = x.new_empty((B, 4, C, H, W))
            _triton_cross_scan[(NH * NW, NC, B)](x, y, BC,
                                                 BH, BW, C, H, W, NH, NW)
            return y.view(B, 4, C, -1)
        ...
        def backward(ctx, y: torch.Tensor):
            ...
            y = y.contiguous().view(B, 4, C, H, W)
            x = y.new_empty((B, C, H, W))
            _triton_cross_merge[(NH * NW, NC, B)](x, y,
                                                  BC, BH, BW, C, H, W, NH, NW)
            return x
